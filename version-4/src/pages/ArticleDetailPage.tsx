...
export default function ArticleDetailPage() {
  ...
  return (
    <>
      <article className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        <header>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-ink-900/10 pb-4 text-sm text-ink-500">
            {article.reporter && (
              <span className="font-semibold text-ink-700">{article.reporter}</span>
            )}
            <span>·</span>
            ...
            <span className="text-ink-300">({formatTimeAgo(article.publishedAt)})</span>
        ...
        <div className="mt-7 flex flex-col gap-5">
          {(article.body ?? [article.excerpt ?? '본문 내용이 준비 중입니다.']).map(
            (paragraph, i) => (
        ...
        <nav className="mt-8 divide-y divide-ink-900/10 border-y border-ink-900/10">
          {next && (
            ...
            >
              <span className="shrink-0 text-xs font-bold text-flash-600">다음기사</span>
              <span className="line-clamp-1 flex-1 text-sm text-ink-800">{next.title}</span>
          ...
          {prev && (
            <Link
              ...
              className="flex items-center gap-3 py-3.5 hover:bg-paper-100"
            >
              <span className="shrink-0 text-xs font-bold text-ink-500">이전기사</span>
              <span className="line-clamp-1 flex-1 text-sm text-ink-800">{prev.title}</span>
              <span className="text-ink-300">›</span>
            </Link>
          )}
        </nav>
      ...
      {/* 관련기사 — 풀와이드 캐러셀 */}
      {related.length > 0 && (
        <NewsCarousel title={`${meta?.label ?? ''} 관련기사`} articles={related} />
      )}
    </>
  )
}
